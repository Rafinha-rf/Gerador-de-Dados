...
sg.theme('dark')
#criar layout da aplicação(esqueleto)
...
layout = [
    ...
    [sg.Button('Gerar Endereços', size=(20,0)), 
...
while True:
    ...
    if event == 'Gerar Nome':
        ...
        with open('dados_de_teste.txt', 'a', encoding='utf-8', newline='') as arquivo:
            arquivo.write(f'NOME: {fake.name()}{os.linesep}PROFISSÃO: {fake.job()}{os.linesep}ENDEREÇO: {fake.address()}{os.linesep}PLACA: {fake.license_plate()}{os.linesep}CARTÃO DE CRÉDITO: {fake.credit_card_full()}{os.linesep}')    
